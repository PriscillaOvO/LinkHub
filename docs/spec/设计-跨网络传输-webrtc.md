...
- **T5 — 信令服务器抗滥用 + 客户端韧性 ✅（2026-06-19 完成，C1 于 2026-06-20 加固）**：见 §7「抗滥用」。服务器 `limits.rs`（帧大小上限 + payload 上限 + 固定窗口速率限制，`serve_with_limits` 可调）；客户端 `connect_with_backoff`（指数退避 `RetryPolicy`）+ `ping()` 心跳。C1 追加跨连接并发防线：全局最大连接数 + 单 IP 并发连接数，accept 后握手前拒绝超限连接，并用 permit 精确释放连接计数。验收：服务器 `RateLimiter`/`ConnectionRegistry` 单测 + 集成测试覆盖超大 payload、消息洪泛、同 IP 超限、全局超限；core 退避/心跳测试保持不变。
