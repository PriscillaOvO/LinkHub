...
- **抗滥用 ✅（T5，2026-06-19 完成）**：信令服务器每连接做三道防线（`limits.rs`）：① **帧/消息大小上限**（默认 64 KiB，协议层 `WebSocketConfig.max_message_size/max_frame_size`，超大帧直接断连，防内存爆破）；② **`payload_hex` 上限**（默认 32K hex=16KiB binary，应用层优雅回 `Error{too large}`、保持会话——SDP 信封才几 KB）；③ **消息速率限制**（固定窗口，默认 40 条/秒，超出回 `Error{rate limit exceeded}` 并断连，防刷流/被当任意中继）。`serve_with_limits(listener, Limits)` 可调，测试用紧配置确定性触发。客户端侧补**重连退避**（`SignalingClient::connect_with_backoff` 指数退避 + `RetryPolicy`）与**心跳** `ping()`（保活 NAT 映射、尽早暴露死链）。
...
- **desktop (Tauri)**：已接线（T6）——`connection_plan` 命令 + `webrtc_send_file`/`webrtc_receive_file`（`spawn_blocking` 不挡 UI 线程，gate 在桌面 `webrtc` feature）+ 发送页跨网络卡片。注意 ICE 收集需要 UDP 出网，企业网防火墙降级到 TURN（T4 已可）。
- **Android**：已接 JNI 桥（T7）——`RustBridge.webrtcSendFile`/`webrtcReceiveFile`（`external fun`，符号常在；`.so` 不带 `webrtc` feature 时回 JSON 错误，带时走真实跨网路径，经共享 `webrtc_session`）。WebRTC/UDP 在 Doze/后台受限——传输期要前台服务（已有），两个调用都阻塞、须在后台线程+前台服务里跑。带 webrtc 的 `.so` 需 `cargo ndk -P 24`（minSdk 24，webrtc-rs 用 `getifaddrs`）。**体积代价见 §9 T7**（gate 在 feature，默认 `.so` 不变重）。push 唤醒离线设备仍是后续问题。
...
- **T5 — 信令服务器抗滥用 + 客户端韧性 ✅（2026-06-19 完成）**：见 §7「抗滥用」。服务器 `limits.rs`（帧大小上限 + payload 上限 + 固定窗口速率限制，`serve_with_limits` 可调）；客户端 `connect_with_backoff`（指数退避 `RetryPolicy`）+ `ping()` 心跳。验收：服务器 3 单测（`RateLimiter`）+ 2 集成（超大 payload 被拒、刷消息被限流断连）；core 3 单测（退避序列/封顶/重试到上限）+ 2 集成（退避连真实服务器成功且 ping 不打断、无服务器时退避后失败）。
- **T6 — 桌面 (Tauri) 集成 ✅（2026-06-19 完成）**：① **抽出共享高层入口**——core 新增 `net/webrtc_session.rs`（feature `webrtc`）：`send_file_over_webrtc` / `receive_file_over_webrtc` 把「信令登录 + 签名 SDP 交换(T3) + DataChannel 建立 + 认证 Noise 传输」打包成两个阻塞调用，CLI/桌面/Android 共用一份桥逻辑（CLI 的 `run_{connect,listen}_webrtc` 改为薄封装委托，删掉 main.rs 里 ~250 行重复桥代码；`webrtc_cli_e2e` 仍绿，证明行为等价）。② **桌面命令**：`connection_plan`（纯函数，按 `plan_connection` 产出 LAN直连→打洞→中继有序路径供 UI 显示，无需 webrtc feature）；`webrtc_send_file` / `webrtc_receive_file`（async Tauri 命令，经 `tauri::async_runtime::spawn_blocking` 把阻塞传输移出 UI 线程，gate 在桌面 `webrtc` feature，关时回友好错误）。③ **前端**：发送页新增「跨网络传输 (WebRTC)」卡片（信令 URL、STUN/TURN、TURN 凭证、仅中继、目标设备、文件、发送/监听/查看路径）。验收：桌面 12 smoke 测试（含 2 个 `connection_plan` 顺序/裁剪）；默认 + `--features webrtc` 两种构建 `cargo check`/`clippy -D warnings` 干净；`node --check` 前端。**未做**：真实双机跨网桌面实测（命令与 UI 已就绪，需两台不同网络机器 + 公网信令）。
- **T7 — Android JNI + on-device webrtc-rs 运行期 + `.so` 体积 ✅（2026-06-19 完成）**：① **JNI 桥**——`jni_bridge.rs` 加 `Java_..._webrtcSendFile`/`webrtcReceiveFile`（符号常在，body gate 在 `webrtc` feature：默认 `.so` 回 JSON 错误、不拉 webrtc-rs/tokio；带 feature 走真实路径，复用 T6 的 `webrtc_session`，接收复用 `onFileReceived` 回调）；ICE 经 JSON `{ice_urls,turn_username,turn_credential,relay_only}` 传入。Kotlin `RustBridge` 加对应 `external fun`。② **构建/运行期**——带 webrtc 的 `.so` 须 `cargo ndk -P 24`（minSdk 24）；webrtc-rs 在 Android 运行期已由 T2（双模拟器 CLI 真跑）背书，本任务证明 JNI 路径**交叉编译干净**（默认双 ABI + `--features webrtc` x86_64 `cargo ndk check` 全过）。③ **`.so` 体积实测（x86_64 release）**：默认 raw 1.56 MiB / strip 1.20 MiB / gzip 0.58 MiB；带 webrtc raw 12.5 MiB / **strip 9.62 MiB** / gzip 3.98 MiB——**strip 后单 ABI +8.4 MiB、压缩进 APK +3.4 MiB**。结论：体积代价大，**坐实「webrtc 默认关、按构建 opt-in」决策**（默认 `.so` 一字节不增）；要发跨网安卓包就 `--features webrtc` 单独出包。**未做**：端侧 Compose UI/前台服务接线调用这两个 `external fun`（符号已就绪）、真机 arm64 JNI 实跑（T2 已证模拟器 x86_64 运行期）。
